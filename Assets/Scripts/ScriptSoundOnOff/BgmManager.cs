...
public class BgmManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if(instance == null)//Se nao existe nenhum objeto da cena que tenha a classe referenciada 
        {
            ...
            DontDestroyOnLoad(instance);//E n�o permite que esse objeto seja destruido entre cenas
            AS = GetComponent<AudioSource>();
            StartCoroutine("StartLoop");
        }
        else//Se ja existe essa classe referenciada
        {
            Destroy(gameObject);//a gente Destroy o gameObject, pois j� existe um deles na cena 
        }
